...
from micecraft.examples.report.Report import Report
...
class ExperimentManager(object):
    ...
    def addReport(self, report: Report, index=None):
        if logging.getLogger().isEnabledFor(logging.DEBUG):
            name_split = report.title.split(">")
            report_name = name_split[len(name_split) // 2]
            logging.debug(f"Adding report: {report_name}")
...
class Experiment(object):
    ...
    def __init__(self, name):
        ...
        if "merged" in self.name:
            self.simpleName = self.name[
                :-7
            ]  # remove the -merged at the end of the name of the experiment
