...
from micecraft.examples.report.Experiment import ExperimentManager
...
class WebSite(object):
    ...
    def miniCard(self, title, content, style="primary"):
        ...
        return self.env.get_template("miniCard.html").render(
    ...
    def renderReportList(
        ...
        for report in reportList:
            ...
            if (
                report.template.lower() != "minicard.html"
            ):  # fixme: if the anchor is present, it generates a line break
            ...
            if report.template.lower() == "minicard.html":
                ...
                if inRow == False:
                    ...
                    inRow = True
                content += self.miniCard(
                    report.title, report.data, style=report.style
                )
                continue
    ...
    def cache(self, file):
        if self.cacheFolder == None:
            logging.error("Website > can't cache: no cache folder set.")
            return
        file = os.path.basename(file)
        fileToCache = self.outFolder + "/" + file
        cacheFile = self.cacheFolder + "/" + file
        logging.info(f"Caching file {file}")
        shutil.copy(fileToCache, cacheFile)

    def useCache(self, file):
        if self.cacheFolder == None:
            logging.error(
                "Website > can't retreive cache: no cache folder set."
            )
            return
        file = os.path.basename(file)
        cacheFile = self.cacheFolder + "/" + file
        if os.path.isfile(cacheFile):
            logging.info(f"Using cache for file {file}")
    ...
    def initWebSiteOutFolder(self):
        ...
        logging.info("Remove existing report files...")
        ...
        # copy initial files
        logging.info("Copy default website ...")
        if self.defaultWebSiteFolder != None:
            ...
            for file in files:
                logging.debug(
                    f"Copy {file} --> {self.outFolder}/{os.path.basename(file)} "
                )
                shutil.copy(
                    f"{file}", f"{self.outFolder}/{os.path.basename(file)}"
                )
    ...
    def generateWebSite(self):
        ...
        names = [experiment.name for experiment in experimentList]
        logging.info(f"Experiment list: {names}")
        ...
        if experimentMain != None:
            content += self.renderReportList(
                experimentMain.reportList, templateFolder, outFolder, "Main"
            )
            experimentMainTimeGenerationInS = (
                experimentMain.getGenerationTimeInS()
            )
        ...
        with open(
            outFolder + "index.html", "w", encoding="utf-8"
        ) as text_file:
            text_file.write(mainHTML)

        # Sub pages generation

        logging.info("Sub page generation...")
        ...
        for experiment in experimentList:
            ...
            with open(
                outFolder + experimentFile, "w", encoding="utf-8"
            ) as text_file:
                text_file.write(mainHTML)
    ...
    def upload(self, localFolder, remoteFolder):
        ...
        if self.passFile == None:
            logging.error("No passfile set. Upload canceled.")
            return
        ...
        logging.debug(f"FTP session: {session}")

        # www/report

        logging.debug(f"Local folder: {localFolder}")
        files = []
        ...
        for f in onServerFiles:
            logging.debug(f"File on server: {f}")
            ...
            if ".mp4" in simpleFile:
                for fileOnline in onServerFiles:
                    if simpleFile in fileOnline:
                        ...
                        logging.info(
                            f"Already existing file on server: Skip upload for file {file}"
                        )
                        break
        ...
        for file in files:
            ...
            logging.info(f"store command: {storeCommand}")
